...
def vids(request, sel_event_id):
    ...
    if request.method == "POST":
        ...
        if vidform.is_valid():
            video = vidform.save()
            logger.info(
                "Added archive video {}".format(video.name),
...
def editvid(request, sel_event_id, video_id):
    ...
    if request.method == "POST":
        ...
        if vidform.is_valid():
            ...
            logger.info(
                "Edited archive video {}".format(r_video.name),
                extra={"user": request.user, "event": event},
            )
            return HttpResponseRedirect(reverse("manage-arkisto:vids", args=(sel_event_id,)))
    else:
...
def deletevid(request, sel_event_id, video_id):
    ...
    try:
        ...
        video.delete()
        logger.info(
            "Deleted archive video {}".format(video.name),
            extra={"user": request.user, "event": event},
        )
...
def editcat(request, sel_event_id, category_id):
    ...
    if request.method == "POST":
        ...
        if catform.is_valid():
            r_cat = catform.save()
            logger.info(
                "Edited archive video category {}".format(r_cat.name),
                extra={"user": request.user, "event": event},
            )
            return HttpResponseRedirect(reverse("manage-arkisto:vidcats", args=(sel_event_id,)))
    else:
...
def deletecat(request, sel_event_id, category_id):
    ...
    try:
        ...
        cat.delete()
        logger.info(
            "Deleted archive video category {}".format(cat.name),
            extra={"user": request.user, "event": event},
        )
