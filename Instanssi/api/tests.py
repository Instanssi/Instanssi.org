...
class APIUnauthenticatedTests(TestCase):
    ...
    def test_store_transaction(self):
        ...
        self.assertEqual(
            self.api.post(url).status_code, 400
        )  # No data = bad req. Tested properly in store tests.
        self.assertEqual(self.api.options(url).status_code, 200)
...
class APIAuthenticatedTests(TestCase):
    def setUp(self):
        ...
        self.event = Event.objects.create(
            name="Instanssi 3000", date=timezone.now(), archived=False, mainurl="http://localhost/"
        )
    ...
    def test_user_vote_codes(self):
        ...
        # Test POST
        req = self.api.post(
            url, data={"event": self.event.id, "ticket_key": self.store_transaction_item.key}
        )
        self.assertEqual(req.status_code, 201)
    ...
    def test_user_vote_code_requests(self):
        ...
        self.assertEqual(req.status_code, 200)
        self.assertListEqual(
            req.data, [{"id": 1, "event": self.event.id, "text": "Test request", "status": 0}]
        )
        ...
        self.assertEqual(req.status_code, 200)
        self.assertDictEqual(
            req.data, {"id": 1, "event": self.event.id, "text": "Test request 2", "status": 0}
        )
    ...
    def test_user_votes(self):
        ...
        req = self.api.post(url, data={"compo": self.compo.id, "entries": [self.compo_entry.id]})
        self.assertEqual(req.status_code, 400)
        ...
        req = self.api.post(url, data={"compo": self.compo_b.id, "entries": [self.compo_entry.id]})
        self.assertEqual(req.status_code, 400)
        self.assertDictEqual(req.data, {"non_field_errors": ["Äänestysoikeus puuttuu"]})

        # Test POST: Get voting rights
        req = self.api.post(
            "/api/v1/user_vote_codes/",
            data={"event": self.event.id, "ticket_key": self.store_transaction_item_b.key},
        )
        self.assertEqual(req.status_code, 201)
        ...
        req = self.api.get(url)
        self.assertEqual(req.status_code, 200)
        self.assertListEqual(req.data, [{"compo": self.compo_b.id, "entries": [self.compo_entry.id]}])
