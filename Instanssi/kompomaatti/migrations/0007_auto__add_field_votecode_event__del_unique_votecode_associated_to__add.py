...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'VoteCode', fields ['associated_to']
        db.delete_unique('kompomaatti_votecode', ['associated_to_id'])

        # Adding field 'VoteCode.event'
        db.add_column('kompomaatti_votecode', 'event',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['kompomaatti.Event'], null=True, blank=True),
                      keep_default=False)

        # Adding unique constraint on 'VoteCode', fields ['event', 'key']
        db.create_unique('kompomaatti_votecode', ['event_id', 'key'])

        # Adding unique constraint on 'VoteCode', fields ['associated_to', 'event']
        db.create_unique('kompomaatti_votecode', ['associated_to_id', 'event_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'VoteCode', fields ['associated_to', 'event']
        db.delete_unique('kompomaatti_votecode', ['associated_to_id', 'event_id'])

        # Removing unique constraint on 'VoteCode', fields ['event', 'key']
        db.delete_unique('kompomaatti_votecode', ['event_id', 'key'])

        # Deleting field 'VoteCode.event'
        db.delete_column('kompomaatti_votecode', 'event_id')

        # Adding unique constraint on 'VoteCode', fields ['associated_to']
        db.create_unique('kompomaatti_votecode', ['associated_to_id'])
