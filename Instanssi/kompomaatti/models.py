...
class VoteCodeRequest(models.Model):
    ...
    user = models.OneToOneField(User, unique=True, verbose_name=u'Käyttäjä', help_text=u'Pyynnön esittänyt käyttäjä')
...
class Compo(models.Model):
    ...
    thumbnail_pref = models.IntegerField(u'Pikkukuvan asetukset', choices=THUMBNAIL_REQ, default=2, help_text=u'Pikkukuvan luonti ja asettaminen.')
...
class Entry(models.Model):
    ...
    imagefile_thumbnail = ImageSpecField([ResizeToFill(160, 100)], source='imagefile_original', format='JPEG', options={'quality': 90})
    imagefile_medium = ImageSpecField([ResizeToFill(640, 400)], source='imagefile_original', format='JPEG', options={'quality': 90})
