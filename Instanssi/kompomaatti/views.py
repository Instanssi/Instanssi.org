...
from Instanssi.kompomaatti.misc.time_formatting import (
    competition_times_formatter,
    compo_times_formatter,
)
...
def compo_details(request, event_id, compo_id):
    ...
    return render(
        request,
        "kompomaatti/compo_details.html",
        {
            "sel_event_id": int(event_id),
            "compo": compo,
            "entryform": entryform,
...
def compo_vote(request, event_id, compo_id):
    ...
    if request.method == "POST":
        ...
        if group:
            ...
            group = VoteGroup.objects.create(user=request.user, compo=compo)
...
def competition_details(request, event_id, competition_id):
    ...
    competition = competition_times_formatter(
        get_object_or_404(Competition, pk=int(competition_id), active=True, event_id=int(event_id))
    )
    ...
    return render(
        request,
        "kompomaatti/competition_details.html",
        {
            "sel_event_id": int(event_id),
...
def entry_details(request, event_id, compo_id, entry_id):
    ...
    return render(
        request,
        "kompomaatti/entry_details.html",
        {
            "sel_event_id": int(event_id),
            "entry": entry,
            "compo": compo,
