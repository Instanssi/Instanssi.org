...
- API base URL: `/api/v2/`
- OpenAPI spec generated via `make openapi` in the backend directory
...
│   ├── services/         # Application-wide state and business logic (auth, events, push)
│   ├── test/             # Test setup + helpers (setup.ts, helpers/)
...
├── package.json
├── vite.config.ts
├── vitest.config.ts      # Separate test runner config
├── tsconfig.json         # Root tsconfig (project references)
├── tsconfig.app.json     # App-code tsconfig
├── tsconfig.node.json    # Node-side (configs) tsconfig
├── tsconfig.sw.json      # Service-worker tsconfig
└── README.md

# Sibling: ../backend/openapi.yaml (generated; consumed by `pnpm run generate-api`)
```
...
Main service composables (in `src/services/`):
...
Reusable view-level composables (in `src/composables/`):
- `useAsyncAction()` - Wraps async UI actions with loading + error state
- `useTableState()` - Persists table sort/filter/pagination state
