...
// Uses client clock — acceptable for admin UI since organizer machines are typically synced
...
const entryDetails = ref<Map<number, CompoEntry>>(new Map());
...
interface DisplayEntry {
    ...
    name: string;
    creator: string;
    imagefile_thumbnail_url: string | null;
    live_voting_revealed: boolean;
}

const displayEntries = computed((): DisplayEntry[] => {
    if (!state.value) return [];
    return state.value.entries.map((e) => {
        const detail = entryDetails.value.get(e.id);
        return {
            id: e.id,
            name: detail?.name ?? `#${e.id}`,
            creator: detail?.creator ?? "",
            imagefile_thumbnail_url: detail?.imagefile_thumbnail_url ?? null,
            live_voting_revealed: e.live_voting_revealed,
        };
    });
});

const nextRevealableEntryId = computed(() => {
    if (!state.value) return null;
    const first = state.value.entries.find((e) => !e.live_voting_revealed);
    return first?.id ?? null;
});

const lastHideableEntryId = computed(() => {
    if (!state.value) return null;
    const revealed = state.value.entries.filter((e) => e.live_voting_revealed);
    if (revealed.length === 0) return null;
    return revealed[revealed.length - 1]!.id;
});
...
async function loadEntries() {
    try {
        const response = await api.adminEventKompomaattiEntriesList({
            ...
            // High limit to fetch all entries in one request; compos rarely exceed ~100 entries
            query: { compo: compoId.value, limit: 1000 },
        });
        if (response.data?.results) {
            const map = new Map<number, CompoEntry>();
            for (const entry of response.data.results) {
                map.set(entry.id, entry);
            }
            entryDetails.value = map;
        }
    } catch (e) {
        console.error(e);
    }
...
function schedulePoll() {
    stopPolling();
    if (pollIntervalMs.value === null) return;
    const jitter = Math.floor(Math.random() * 500);
    pollTimer = setTimeout(async () => {
        await loadState();
        schedulePoll();
    }, pollIntervalMs.value + jitter);
}
...
onMounted(async () => {
    loading.value = true;
    await Promise.all([loadCompoName(), loadEntries(), loadState()]);
    loading.value = false;
    schedulePoll();
});
