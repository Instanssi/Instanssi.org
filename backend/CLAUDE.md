...
**IMPORTANT for AI Agents**: Always run `isort .` and `black .` after making code changes to ensure consistent formatting. The project uses these tools to maintain code style, and they should be run before considering a task complete.
...
**APIv2 Structure:**
- `viewsets/user/`: Endpoints for data bound to and available to the currently logged-in user (e.g., user's own entries, user info)
- `viewsets/auth/`: Authentication-related endpoints (login, logout, social auth)
- Other viewsets (e.g., `kompomaatti/`, `events.py`): Public or permission-based endpoints using Django's permission system
...
## Workflow Guidelines for AI Agents

When making changes to the codebase, follow this workflow:

1. **Make your code changes** (create, edit, or delete files)
2. **Run code formatters** (REQUIRED):
   ```bash
   isort .
   black .
   ```
...
**Why this matters**: The project enforces strict code formatting standards. Running `isort` and `black` ensures:
- Consistent import ordering across the codebase
- Uniform code style (line length, spacing, etc.)
- No formatting-related CI/CD failures
- Easy code reviews and maintainability
