...
class UserCompetitionParticipationSerializer(ModelSerializer):
    ...
    def validate_competition(self, competition: Competition) -> Competition:
        ...
        # Make sure competition is scoped to the event from the URL
        request = self.context.get("request")
        view = self.context.get("view")
        if view and hasattr(view, "kwargs"):
            event_pk = view.kwargs.get("event_pk")
            if event_pk and competition.event_id != event_pk:
                raise ValidationError("Competition does not belong to this event")
    ...
    def validate(self, data: dict) -> dict:
        ...
        if not competition.is_participating_open():
            raise ValidationError("Competition participation time has ended")
        ...
        if request and request.user:
            ...
            if not self.instance or has_changed:
                ...
                if existing:
                    raise ValidationError("You have already participated in this competition")
