...
class LiveVotingViewSet(GenericViewSet[LiveVotingState]):
    ...
    def _get_state(self, compo: Compo, *, for_update: bool = False) -> LiveVotingState:
        state, _ = LiveVotingState.objects.get_or_create(compo=compo)
        if for_update:
            state = LiveVotingState.objects.select_for_update().get(pk=state.pk)
        return state
    ...
    def partial_update(self, request: Request, **kwargs: Any) -> Response:
        compo = self._get_compo()
        state = self._get_state(compo, for_update=True)
    ...
    def reveal_entry(self, request: Request, **kwargs: Any) -> Response:
        ...
        if not entry.live_voting_revealed:
            next_entry = (
                Entry.objects.filter(compo=compo, live_voting_revealed=False)
                .order_by("order_index", "id")
                .first()
            )
            if next_entry is None or next_entry.pk != entry.pk:
                raise ValidationError(_("Entries must be revealed in order."))
    ...
    def hide_entry(self, request: Request, **kwargs: Any) -> Response:
        ...
        if entry.live_voting_revealed:
            last_revealed = (
                Entry.objects.filter(compo=compo, live_voting_revealed=True)
                .order_by("-order_index", "-id")
                .first()
            )
            if last_revealed is None or last_revealed.pk != entry.pk:
                raise ValidationError(_("Only the last revealed entry can be hidden."))
            Entry.objects.filter(pk=entry.pk).update(live_voting_revealed=False)
    ...
    def reveal_all(self, request: Request, **kwargs: Any) -> Response:
        ...
        Entry.objects.filter(compo=compo, live_voting_revealed=False).update(live_voting_revealed=True)
    ...
    def hide_all(self, request: Request, **kwargs: Any) -> Response:
        ...
        Entry.objects.filter(compo=compo).update(live_voting_revealed=False)
        state.current_entry = None
        state.save()
