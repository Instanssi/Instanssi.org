...
class StoreItem(models.Model):
    ...
    description = SanitizedHtmlField(_("Product description"))
    ...
    max_per_order = models.IntegerField(_("Maximum per order"), default=10)
    sort_index = models.IntegerField(_("Sort order"), default=0)
    discount_amount = models.IntegerField(_("Discount threshold"), default=-1)
    discount_percentage = models.IntegerField(_("Discount percentage"), default=0)
    is_ticket = models.BooleanField(_("Product is a ticket"), default=False)
    is_secret = models.BooleanField(_("Product is secret"), default=False)
...
class StoreItemVariant(models.Model):
    ...
    name = models.CharField(_("Variant name"), max_length=32, blank=False, null=False)
...
class StoreTransaction(models.Model):
    ...
    time_created = models.DateTimeField(_("Created at"), null=True, blank=True, db_index=True)
    time_paid = models.DateTimeField(_("Payment confirmed at"), null=True, blank=True)
    time_pending = models.DateTimeField(_("Payment initiated at"), null=True, blank=True)
    time_cancelled = models.DateTimeField(_("Cancelled at"), null=True, blank=True)
    ...
    mobile = models.CharField(_("Mobile phone"), max_length=64, blank=True)
...
class StoreTransactionEvent(models.Model):
    ...
    message = models.CharField(_("Event message"), max_length=255, null=False)
    data = models.JSONField(_("Event data"), null=True)
    created = models.DateTimeField(_("Created at"), null=False, default=timezone.now)
...
class TransactionItem(models.Model):
    key = models.CharField(_("Key"), max_length=40, unique=True)
    item = models.ForeignKey(StoreItem, verbose_name=_("Product"), on_delete=models.PROTECT)
    variant = models.ForeignKey(
        StoreItemVariant, verbose_name=_("Product variant"), null=True, on_delete=models.PROTECT
    )
    transaction = models.ForeignKey(
        StoreTransaction, verbose_name=_("Transaction"), on_delete=models.CASCADE
    )
    time_delivered = models.DateTimeField(_("Delivery time"), null=True, blank=True)
    purchase_price = models.DecimalField(_("Product price"), max_digits=5, decimal_places=2)
    original_price = models.DecimalField(
        _("Original product price"),
        max_digits=5,
        decimal_places=2,
    )
...
class Receipt(models.Model):
    ...
    mail_to = models.CharField(_("Recipient address"), max_length=256)
    mail_from = models.CharField(_("Sender address"), max_length=256)
    sent = models.DateTimeField(_("Sent at"), default=None, null=True)
    params = models.TextField(_("Send parameters"), default=None, null=True)
    content = models.TextField(_("Receipt content"), default=None, null=True)
