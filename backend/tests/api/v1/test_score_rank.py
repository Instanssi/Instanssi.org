"""Tests for score and rank calculations in API v1 endpoints.

These tests validate the actual calculated score and rank values,
which is critical for verifying correctness when moving score/rank
algorithms to the database side.
"""
...
class TestCompoEntryScoreRank:
    """Test compo entry score and rank calculations via v1 API."""

    def test_scores_hidden_when_show_voting_results_false(self, api_client, votable_compo_entry):
        """When show_voting_results=False, score and rank are None."""
    ...
    def test_vote_based_scores_and_ranking(
        ...
        # Expected scores: entry1=2.0, entry2=0.5, entry3≈0.833
        # Ranking: entry1=1st, entry3=2nd, entry2=3rd
    ...
    def test_archive_score_rank_override(self, api_client, closed_compo, closed_compo_entry):
        ...
        req = api_client.get(f"/api/v1/compo_entries/{closed_compo_entry.id}/")
        assert req.status_code == 200
...
class TestCompetitionParticipationScoreRank:
    """Test competition participation score and rank via v1 API."""

    def test_scores_hidden_when_show_results_false(self, api_client, competition_participation):
        """When show_results=False, score and rank are None."""
    ...
    def test_ranking_by_score_descending(self, api_client, results_competition, base_user, normal_user):
        """With default score_sort=0, highest score ranks first. Score is formatted string."""
        ...
        assert parts["High Score"]["score"] == "200.0 p"
        assert parts["High Score"]["rank"] == 1
        assert parts["Low Score"]["score"] == "50.0 p"
    ...
    def test_ranking_by_score_ascending(self, api_client, event, base_user, normal_user):
        ...
        assert parts["Fast Runner"]["score"] == "10.5 s"
        assert parts["Fast Runner"]["rank"] == 1
        assert parts["Slow Runner"]["score"] == "99.0 s"
