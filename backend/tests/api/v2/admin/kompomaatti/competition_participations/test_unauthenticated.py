...
def test_unauthenticated_competition_participations_list(api_client, event, method, status):
...
def test_unauthenticated_competition_participations_detail(
    api_client, event, started_competition_participation, method, status
):
    """Test unauthenticated access to participation detail endpoint (Not logged in)."""
    base_url = get_base_url(event.id)
