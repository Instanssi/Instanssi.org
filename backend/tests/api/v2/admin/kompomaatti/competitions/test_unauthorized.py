...
def test_unauthorized_competitions_detail(auth_client, event, competition, method, status):
    """Test unauthorized access to competition detail endpoint (Logged in, but no permissions)."""
    base_url = get_base_url(event.id)
