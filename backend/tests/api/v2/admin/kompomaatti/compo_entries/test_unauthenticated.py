...
def test_unauthenticated_compo_entries_list(api_client, event, method, status):
...
def test_unauthenticated_compo_entries_detail(api_client, event, votable_compo_entry, method, status):
    """Test unauthenticated access to entry detail endpoint (Not logged in)."""
    base_url = get_base_url(event.id)
