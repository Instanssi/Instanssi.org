...
def test_get_state_with_entries(staff_api_client, event, live_voting_compo, live_voting_entries):
    ...
    for entry_data in resp.data["entries"]:
        ...
        assert entry_data["live_voting_order"] == 0
...
def test_reveal_entry(staff_api_client, event, live_voting_compo, live_voting_entries):
    ...
    assert entry.live_voting_order == 1


@pytest.mark.django_db
def test_reveal_multiple_entries_assigns_order(
    staff_api_client, event, live_voting_compo, live_voting_entries
):
    """Revealing entries sequentially assigns increasing order."""
    url = f"{get_base_url(event.id, live_voting_compo.id)}reveal_entry/"

    staff_api_client.post(url, {"entry_id": live_voting_entries[0].id}, format="json")
    staff_api_client.post(url, {"entry_id": live_voting_entries[1].id}, format="json")

    live_voting_entries[0].refresh_from_db()
    live_voting_entries[1].refresh_from_db()
    assert live_voting_entries[0].live_voting_order == 1
    assert live_voting_entries[1].live_voting_order == 2
...
def test_reveal_already_revealed_entry(staff_api_client, event, live_voting_compo, live_voting_entries):
    ...
    entry.refresh_from_db()
    original_order = entry.live_voting_order

    resp = staff_api_client.post(url, {"entry_id": entry.id}, format="json")
    assert resp.status_code == 200
    entry.refresh_from_db()
    assert entry.live_voting_order == original_order
...
def test_hide_entry(staff_api_client, event, live_voting_compo, live_voting_entries):
    ...
    assert entry.live_voting_order == 0
