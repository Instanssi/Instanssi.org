...
def test_unauthorized_uploads_list(auth_client, event, method, status):
...
def test_unauthorized_uploads_nonexistent_detail(auth_client, event, method, status):
...
def test_unauthorized_uploads_existing_detail(auth_client, uploaded_file, method, status):
