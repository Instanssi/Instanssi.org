...
def test_invalid_entry_file_format_rejected(staff_api_client, open_compo, base_user, source_zip, image_png):
...
@pytest.mark.django_db
def test_invalid_source_file_format_rejected(staff_api_client, open_compo, base_user, entry_zip, image_png):
...
@pytest.mark.django_db
def test_invalid_image_file_format_rejected(
...
@pytest.mark.django_db
def test_entry_file_too_large_rejected(staff_api_client, open_compo, base_user, source_zip, image_png):
...
@pytest.mark.django_db
def test_image_required_but_missing_rejected(staff_api_client, open_compo, base_user, entry_zip, source_zip):
...
@pytest.mark.django_db
def test_image_not_allowed_but_provided_rejected(
    staff_api_client, open_compo, base_user, entry_zip, source_zip, image_png
):
...
@pytest.mark.django_db
def test_multiple_file_errors_aggregated(staff_api_client, open_compo, base_user, test_image):
...
@pytest.mark.django_db
def test_entry_copied_to_image_when_configured(
...
def test_staff_can_upload_to_closed_compo(
    ...
    """Test that staff can upload entries to closed compos (no deadline checks)"""
    ...
    req = staff_api_client.post(
        ...
        data={
            ...
            "name": "Late Entry",
            "description": "Staff can add entries after deadline",
            "creator": "Admin",
            "platform": "Any",
            "entryfile": entry_zip,
            "imagefile_original": image_png,
