...
@pytest.mark.django_db
def test_cannot_see_other_users_participations(auth_client, started_competition, normal_user):
    """Test that users can only see their own participations."""
    # Create a participation for a different user
    other_participation = CompetitionParticipation.objects.create(
        competition=started_competition,
        user=normal_user,
    ...
    assert other_participation.id not in participation_ids


@pytest.mark.django_db
def test_cannot_access_participation_in_inactive_competition(auth_client, inactive_competition, base_user):
    """Test that participations in inactive competitions are not accessible."""
    # Create a participation in inactive competition
    participation = CompetitionParticipation.objects.create(
        competition=inactive_competition,
        user=base_user,
    ...
    assert req.status_code == 200
    participation_ids = [p["id"] for p in req.data]
    assert participation.id not in participation_ids

    # Direct access should return 404
    req = auth_client.get(f"{base_url}{participation.id}/")
...
@pytest.mark.django_db
def test_filter_by_competition(auth_client, competition, started_competition, base_user):
    """Test filtering participations by competition."""
    # Create participations in both competitions (using competition that's still open)
    p1 = CompetitionParticipation.objects.create(
    ...
    base_url = get_base_url(competition.event_id)
    req = auth_client.get(f"{base_url}?competition={competition.id}")
    assert req.status_code == 200
    # Response is a list when no pagination params
    assert len(req.data) == 1
    assert req.data[0]["id"] == p1.id
