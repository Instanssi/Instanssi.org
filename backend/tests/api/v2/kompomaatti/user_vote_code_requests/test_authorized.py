...
@pytest.mark.django_db
def test_cannot_see_other_users_requests(auth_client, event, normal_user):
    """Test that users can only see their own vote code requests."""
    # Create a request for a different user
    other_request = VoteCodeRequest.objects.create(
        event=event,
        user=normal_user,
...
@pytest.mark.django_db
def test_cannot_access_other_users_request(auth_client, event, normal_user):
    ...
    base_url = get_base_url(event.id)
    req = auth_client.get(f"{base_url}{other_request.id}/")
    assert req.status_code == 404


@pytest.mark.django_db
def test_filter_by_status(auth_client, event, base_user):
    """Test filtering requests by status."""
    # Create an approved request
    approved_request = VoteCodeRequest.objects.create(
        event=event,
        user=base_user,
    ...
    assert req.status_code == 200
    assert len(req.data) == 1
    assert req.data[0]["id"] == approved_request.id
