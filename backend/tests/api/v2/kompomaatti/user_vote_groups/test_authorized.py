...
def test_create_votes_with_approved_request(
    auth_client, event, votable_compo, votable_compo_entry, base_user
):
    """Test that users can vote with an approved vote code request."""
    # Create an approved vote code request
...
def test_cannot_vote_with_rejected_request(
    ...
    # Create a rejected vote code request
    VoteCodeRequest.objects.create(
        event=event,
        user=base_user,
        text="Please give me voting rights",
...
@pytest.mark.django_db
def test_cannot_see_other_users_votes(auth_client, votable_compo, normal_user):
    """Test that users can only see their own vote groups."""
    # Create a vote group for a different user
    other_vote_group = VoteGroup.objects.create(
        compo=votable_compo,
        user=normal_user,
    )
    ...
    assert other_vote_group.id not in vote_group_ids


@pytest.mark.django_db
def test_filter_by_compo(auth_client, ticket_vote_code, votable_compo, votable_compo_entry, base_user):
    """Test filtering vote groups by compo."""
    # Create a vote group
    vote_group = VoteGroup.objects.create(compo=votable_compo, user=base_user)
    vote_group.create_votes([votable_compo_entry])

    base_url = get_base_url(votable_compo.event_id)
    req = auth_client.get(f"{base_url}?compo={votable_compo.id}")
    assert req.status_code == 200
    assert len(req.data) == 1
    assert req.data[0]["id"] == vote_group.id
