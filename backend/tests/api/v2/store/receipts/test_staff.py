...
def get_base_url(event_id):
    return f"/api/v2/event/{event_id}/store/receipts/"
...
@pytest.mark.django_db
def test_unauthenticated_cannot_access_receipts(api_client, store_item):
    """Test that unauthenticated users cannot access receipts endpoint."""
...
@pytest.mark.django_db
def test_unauthorized_cannot_access_receipts(auth_client, store_item):
    """Test that users without permissions cannot access receipts endpoint."""
    base_url = get_base_url(store_item.event_id)
